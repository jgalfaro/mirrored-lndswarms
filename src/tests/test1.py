...
def init_test():
    ...
    #clight = clightning_btcController()
    ...
    print '```'
    cmd_out = lnd.lnd_nodes[0].open_channel(lnd.lnd_nodes[1].identity_pubkey, CHAN_VALUE)
    ...
    for i in range(MAX_CONFIRMATION_BLKS):
        blk = blockchain.generate_blocks(1, True)
    ...
    lnd.lnd_nodes[1].channel_balance()
    print '```'
