...
# -*- coding:utf-8 -*-
__author__ = 'ender'
from bitcoindController import *
from lnd_btcController import *
from clightning_btcController import *
from eclair_btcController import *
...
def init_test():
    ...
    """
    lnd_nodes_name = {0: {'id': "M"}, 1: {'id': "A"}, 2: {'id': "B_1"}}
    clightning_nodes_name = {0: {'id': "B_4"}}
    eclair_nodes_name = {0: {'id': "B_7"}}
    """
    ...
    print '```'
    cmd_out = lnd.lnd_nodes[0].open_channel(lnd.lnd_nodes[1].identity_pubkey, M_A_CHAN_VALUE)
    M_A_channel_funtxid = cmd_out["funding_txid"]
    print '```'

    M_A_channel_output_index = ""
    for i in range(1, MAX_CONFIRMATION_BLKS, 5):
        blockchain.generate_blocks(i, True)
        cmd_out = lnd.lnd_nodes[0].list_channels(True)
        for channel in cmd_out["channels"]:
            if (M_A_channel_funtxid in channel["channel_point"]) and (channel["active"]):
                M_A_channel_output_index = channel["channel_point"].split(':')[1]
                break
        if M_A_channel_output_index is not "":
            break
        time.sleep(5)
    M_A_channel_output_index = "0" if M_A_channel_output_index is "" else M_A_channel_output_index
    print "- Generated " + str(i) + " block so the channel is confirmed and active:"
    ...
    for bi_node in bi_nodes:
        ...
        print '```'
        cmd_out = lnd.lnd_nodes[1].open_channel(bi_node.identity_pubkey, A_Bi_CHAN_VALUE)
        A_Bi_channel_funtxid = cmd_out["funding_txid"]
        print '```'

        A_Bi_channel_output_index = ""
        for j in range(1, MAX_CONFIRMATION_BLKS, 5):
            blockchain.generate_blocks(j, True)
            cmd_out = lnd.lnd_nodes[1].list_channels(True)
            for channel in cmd_out["channels"]:
                if (A_Bi_channel_funtxid in channel["channel_point"]) and (channel["active"]):
                    A_Bi_channel_output_index = channel["channel_point"].split(':')[1]
                    break
            if A_Bi_channel_output_index is not "":
                break
            time.sleep(5)

        A_Bi_channel_output_index = "0" if A_Bi_channel_output_index is "" else A_Bi_channel_output_index
    ...
    for bi_node in bi_nodes:
        ...
        print '```'
        lnd.lnd_nodes[0].channel_balance()
        print '```'

        print "##  Check channel balance info on B_"+str(i)
        print '```'
        bi_node.channel_balance()
        print '```'
